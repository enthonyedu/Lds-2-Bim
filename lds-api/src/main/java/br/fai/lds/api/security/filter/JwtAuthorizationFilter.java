...
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Ocorreu Erro inesperado, foi mal cria");
        } finally {
    ...
    private Jws<Claims> validateToken(HttpServletRequest request) {
        String jwsToken = request.getHeader(ApiSecurityConstants.HEADER)
                .replace(ApiSecurityConstants.PREFIX, "");

        return Jwts.parserBuilder().setSigningKey(ApiSecurityConstants.KEY)
                .build()
